...
/*   Updated: 2023/07/20 17:25:14 by fahmadia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_all_images_to_window(t_game *game, int x, int y)
{
	...
	else if (game->map->plan[y][x] == ENEMY)
		put_enemy_in_window(game, x, y);
}
...
void	create_a_window(t_map *map)
{
	...
	if (game.map->enemy)
		mlx_loop_hook(window, handle_enemy_move_new, &game);
	mlx_loop(window);
	delete_textures_and_images(&game);
